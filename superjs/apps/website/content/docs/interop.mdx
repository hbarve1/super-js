...
| `interface` / `type` aliases | `type` (brace or alias form) | TS `interface X {}` → SJS `type X {}`; `type` aliases stay aliases |
| `<T>`, `<T extends U>`, `<T = D>` | preserved | Generics, constraints, defaults |
| `enum` | string/number union | `enum E { A, B }` → `"A" \| "B"` |
| `A & B` intersection | merged object type, else `dynamic` | Auto-merges when fields don't conflict |
