...
| `A & B` intersection types | Unsound merging — use interfaces with `extends` |
...
Interfaces describe structural shapes. SJS uses structural typing — any value that satisfies the shape is compatible.
...
SJS is structurally typed. A value satisfies an interface if it has the required shape — no `implements` needed at call sites.
