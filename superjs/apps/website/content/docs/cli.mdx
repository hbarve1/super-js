...
Templates: `node-cli`, `fastify-api`, `workers-api`, `lambda-handler`. Each
...
Recompile an input tree and **byte-diff** the emitted JavaScript against an
expected output tree — the build-determinism gate as a user-auditable command.
Exits non-zero on any difference or missing file. Source maps are disabled so the
...
A best-effort **textual** pass: each `.ts` becomes a `.sjs` with `any` rewritten
