...
TypeScript forms SuperJS doesn't model (intersection, conditional, mapped,
`infer`, `any`) degrade to `dynamic`, and top-level functions, classes and
namespaces are reported as not-yet-translated — every dropped or degraded
construct is surfaced as a warning, never silently lost.
...
The same `dynamic` fallbacks as `translate` apply and are reported as warnings.
